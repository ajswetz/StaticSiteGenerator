...
def split_nodes_link(old_nodes):
    ...
    for node in old_nodes:
        ...
        if node.text_type == "text":
            ...
            raw_text = node.text
            if extract_md_link(raw_text) == None:
                ...
                keep_going = True
                while keep_going:
                    link_match = extract_md_link(raw_text)
                    full_link_string = f"[{link_match[0]}]({link_match[1]})"
                    split_text = raw_text.split(full_link_string)
                    ...
                    new_nodes.append(TextNode(text=link_match[0], text_type="link", url=link_match[1]))
